...
BmpImageMod::BMP_IMAGE_PIX_FMT BmpImageMod::begin(File& myFile) {
  uint16_t data16;
  uint32_t data32;
  int n = 0;
...
BmpImageMod::BMP_IMAGE_PIX_FMT BmpImageMod::begin(BmpImageMod::BMP_IMAGE_PIX_FMT fmt
...
BmpImageMod::BMP_IMAGE_PIX_FMT BmpImageMod::convertPixFormat(BmpImageMod::BMP_IMAGE_PIX_FMT fmt) {
...
uint8_t* BmpImageMod::getBmpBuff() {
  return m_bmp_buff;
}

uint8_t* BmpImageMod::getImgBuff() {
  return m_img_buff;
}

int BmpImageMod::getBmpSize() {
  return m_bfSize;
}

int BmpImageMod::getImgSize() {
  return m_biSizeImage;
}

int BmpImageMod::getWidth() {
  return m_biWidth;
}

int BmpImageMod::getHeight() {
  return m_biHeight;
}

BmpImageMod::BMP_IMAGE_PIX_FMT BmpImageMod::getPixFormat() {
  return m_biBitCount/8;
}

/* private functions */
uint8_t* BmpImageMod::swap16(uint16_t word16) {
  m_word16[0] = (uint8_t)(word16 & 0x00ff);
  m_word16[1] = (uint8_t)((word16 & 0xff00) >> 8);
  return m_word16;
}

uint8_t* BmpImageMod::swap32(uint32_t dword32) {
...
uint16_t BmpImageMod::rswap16(uint8_t word16[2]) {
  uint16_t l_data = (uint16_t)(word16[0]);
  uint16_t u_data = (uint16_t)(word16[1]);
  return u_data << 8 | l_data;
}

uint32_t BmpImageMod::rswap32(uint8_t word32[4]) {
  uint32_t data32;
  uint32_t u_data16;
  uint32_t l_data16;
  uint16_t l_data;
